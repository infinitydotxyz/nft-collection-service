...
export async function createCollection(): Promise<void> {
  ...
  const [, , address, chainId, hasBlueCheckArg, resetArg, indexInitiator, partialArg, mintDataArg] = process.argv;
  const hasBlueCheck = hasBlueCheckArg === 'true';
  const reset = resetArg === 'true';
  const partial = partialArg === 'true';
  const mintData = mintDataArg === 'true';
  ...
  await create(address, chainId, hasBlueCheck, reset, indexInitiator, partial, mintData, log);
}
