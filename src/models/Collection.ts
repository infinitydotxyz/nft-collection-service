...
export default class Collection {
  ...
  async getTokensFromMints(fromBlock?: number, toBlock?: number, emitter?: Emittery<{'token': Token}>): Promise<{ tokens: Token[]; numTokens: number, tokensWithErrors: Array<{ error: any, tokenId: string}> }>  {
    ...
    const promise = new Promise<{ collection: CollectionType; tokens: Token[], tokensWithErrors: Array<{ error: any, tokenId: string}> }>(async (resolve, reject) => {
      try{
        ...
        const collection: CollectionType = {
          ...
          owner: deployer.address.toLowerCase(), // note - this is may not be the current owner
