...
export async function updateCollectionNumOwners(emitter: BackgroundTaskEmitter): Promise<void> {
    ...
    if (collection?.metadata?.links?.slug) {
      try {
        ...
        successful += 1;
          ...
          void emitter.emit('update', { error: `Failed to get collection stats: ${collection.chainId}:${collection.address}. Error: ${err?.toString?.()}`} );
      }
    }
  }
  ...
  void emitter.emit('update', { message: `Successfully updated: ${successful} collections. Failed to updated: ${failed} collections`} );
